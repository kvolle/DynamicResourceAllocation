function threshold =set_threshold(target_pk,targeted)
    ...
    for i =1:length(Pk)
        threshold(i) = (log((1-Pk(i))/(1-target_pk(i)))/log(0.3625))*isreal(log(1-0.3625^(sum(i)-1)-target_pk(i)))/20;%2*log(0.3625)/log(Pk(i)-target_pk(i))*isreal(log(0.3625)/log(Pk(i)-target_pk(i)));
    end
end